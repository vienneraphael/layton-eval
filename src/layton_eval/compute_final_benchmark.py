...
def get_ppi_inputs(df_ppi: pl.DataFrame, provider: str, model: str, field_name="both_correct"):
    ...
    df_labelled_unsampled = df_ppi.filter((pl.col("provider") != provider) & (pl.col("human_answer_correct").is_not_null()))
    df_unlabelled = df_ppi.filter(pl.col("model") == model)
    distribution = df_unlabelled.get_column(judge_field_name).value_counts()
    n_samples = distribution.with_columns(
        pl.col("count").alias("n_samples")
    ).select(judge_field_name, "n_samples").to_dicts()
    sub_dfs = []
    for elem in n_samples:
        value, count = elem[judge_field_name], elem["n_samples"]
        sub_dfs.append(
            df_labelled_unsampled.filter(pl.col(judge_field_name) == value).sample(count, with_replacement=True, shuffle=True)
        )
...
def get_benchmark_results(df_ppi: pl.DataFrame, field_name="both_correct") -> pl.DataFrame:
    ...
    for provider, model in df_ppi.select("provider", "model").unique().iter_rows():
        ...
        results["score"].append((ppi_pointestimate[0]*100).round(2))
        spread = ppi_ci_upper - ppi_ci_lower
...
def compute_final_ranks(results_df: pl.DataFrame) -> pl.DataFrame:
    ...
    rank_spread = results_df.join(results_df, how="cross",suffix="_other").group_by("model").agg(
        (pl.col("ppi_ci_lower_other") > pl.col("ppi_ci_upper")).sum().add(1).alias("best_possible_rank"),
        (pl.col("ppi_ci_upper_other") >= pl.col("ppi_ci_lower")).sum().alias("worst_possible_rank")
    )
    return results_df.join(rank_spread, on="model", how="left").sort(by="rank").drop("ppi_ci_lower", "ppi_ci_upper")

def main(field_name: str):
    df_ppi = pl.read_ndjson("hf://datasets/rvienne/layton-eval-ppi/ppi_llm.jsonl", infer_schema_length=100000)
